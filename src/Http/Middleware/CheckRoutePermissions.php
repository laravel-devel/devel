...
namespace Devel\Http\Middleware;
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if (!auth()->user()->hasPermissions($permissions)) {
            ...
            if (back()->getTargetUrl() === url()->current()) {
                ...
                $isAdminDashboard = Route::has('dashboard.index')
                    && Str::startsWith(url()->current(), route('dashboard.index'))
                    && auth()->user()->hasPermissions('admin_dashboard.access')
                    && $dm
                    && $dm->isInstalled()
                    && $dm->isEnabled();
